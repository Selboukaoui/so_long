...
/*   Updated: 2025/03/05 03:32:38 by selbouka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_map(t_game *data, char *path)
{
	...
	if (ft_strlen(path) < 4 || !ft_strcmp(path + ft_strlen(path) - 4, ".ber" ))
		map_err(25, -1, "Error\nPath is invalid\n");
	...
	while (line)
	{
		if (!lines_equal(line, data->colums))
			map_err(25, fd, "Error\nMap is Invalid Check lines\n");
		line = get_next_line(fd);
		data->rows++;
	}
	close (fd);
	if (data->rows > 28 || data->colums > 51)
		map_err(25, -1, "Error\nMap Size is Invalid");
}
